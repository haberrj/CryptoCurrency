...
class Crypto:
    ...
    def decode_csv(self):
        ...
        with open(self.history_file) as csvfile:
            ...
            for value in history_values: 
                holder.append(value)
            size = len(holder)
            if(size > 300): # this way the data will be more relevant ~24h
                for i in range(size-300, size): # doing this to above gives problems due to DictReader
    ...
    def format_dict_values(self):
        ...
        for row in self.dict_values:
            ...
            sub_dict["time"] = int(float(row['time']))
